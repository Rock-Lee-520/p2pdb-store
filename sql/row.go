...
func RowIterToRows(ctx *Context, i RowIter) ([]Row, error) {
	debug.Dump("========= RowIterToRows start ")
	var rows []Row
	for {
		row, err := i.Next(ctx)
		if err == io.EOF {
...
func (i *sliceRowIter) Next(*Context) (Row, error) {
	debug.Dump("========sliceRowIter row Next start")
	if i.idx >= len(i.rows) {
		return nil, io.EOF
	}
	...
	r := i.rows[i.idx]
	i.idx++
	debug.Dump("======== sliceRowIter row Next end")
	return r.Copy(), nil
}
