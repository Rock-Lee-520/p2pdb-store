...
func (d Database) CreateTable(ctx *sql.Context, name string, schema sql.PrimaryKeySchema) error {
	...
	for i, col := range schema.Schema {
		...
		debug.Dump("CreateTable+ stmt is ")
		debug.Dump(stmt)
		if !col.Nullable {
	...
	debug.Dump(fmt.Sprintf("CREATE TABLE `%s` (\n%s\n)",
		name, strings.Join(colStmts, ",\n")))
	sql := "CREATE TABLE   IF NOT EXISTS `mytable3`  (`name` text NOT NULL,`email` text NOT NULL,`phone_numbers` json NOT NULL,`created_at` timestamp NOT NULL)"
	d.shim.Exec(d.name, sql)
