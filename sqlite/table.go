...
func (t *Table) PartitionRows(ctx *sql.Context, partition sql.Partition) (sql.RowIter, error) {
	...
	for QueryRows.Next() {
		...
		err := QueryRows.Scan(is...)
		if err != nil {
			return nil, err
		}

		copyQueryRows = append(copyQueryRows, receiver)
	...
	// }
	//debug.Dump(rows)
	_, ok := t.partitions[string(partition.Key())]
	...
	// rowsCopy := make([]sql.Row, len(rows))
	// copy(rowsCopy, rows)
	rowsCopy := make([]sql.Row, len(copyQueryRows))
	copy(rowsCopy, copyQueryRows)
...
func (t *Table) Inserter(ctx *sql.Context) sql.RowInserter {
	debug.Dump("========Inserter p2pdb-store")
	// debug.Dump(ctx)
	// debug.Dump("========Inserter p2pdb-store")
...
func rowsAreEqual(ctx *sql.Context, schema sql.Schema, left, right sql.Row) (bool, error) {
	debug.Dump("=====rowsAreEqual method")
...
func (t *Table) IndexKeyValues(
	...
	debug.Dump("========IndexKeyValues start ")
...
func (i *partitionIndexKeyValueIter) Next(ctx *sql.Context) (sql.Partition, sql.IndexKeyValueIter, error) {
	debug.Dump("========partitionIndexKeyValueIter Next")

	// ctx.SetRawStatement("t2")
	// debug.Dump(ctx.RawStatement())
...
func (i *indexKeyValueIter) Next(ctx *sql.Context) ([]interface{}, []byte, error) {
	debug.Dump("========= indexKeyValueIter Next start")
	...
	i.pos++
	debug.Dump("=========indexKeyValueIter  Next end")
